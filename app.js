...
class PresentationApp {
    ...
    init() {
        console.log('PresentationApp initialized. Setting up events and initial state.');
        this.bindEvents();
        ...
        // Assegurar que o primeiro slide está visível e ativo após a inicialização
        const initialSlide = document.querySelector(`[data-slide="1"]`);
        if (initialSlide) {
            ...
            initialSlide.style.pointerEvents = 'auto';
            console.log('Initial slide (1) set to active.');
        } else {
            console.error('Initial slide (1) not found in DOM!');
        }
    }
    ...
    goToSlide(slideNumber, direction = 'none') {
        ...
        if (currentSlideElement) {
            ...
            if (direction === 'next') {
                ...
                // Se não houver direção específica (ex: salto direto), apenas fade out
                currentSlideElement.style.transform = 'translateX(0)'; // Mantê-lo centrado enquanto desvanece
            }
            currentSlideElement.style.opacity = '0'; // Desvanece

            // Ocultar o slide atual completamente após a sua transição
            const hideCurrent = () => {
                console.log('[goToSlide] Transition ended for current slide. Hiding...');
                currentSlideElement.style.visibility = 'hidden'; // Tornar invisível
            ...
            // Adicionar o listener para 'transitionend'. Usar um setTimeout de fallback caso a transição não seja detetada.
        ...
        newSlideElement.style.visibility = 'visible'; // Torná-lo visível (ainda transparente e fora do ecrã)
        ...
        // (visibility:visible, opacity:0, transform:X%) ANTES de aplicar o estado final.
        // Sem isto, a transição pode não ocorrer.
        // Usar requestAnimationFrame para garantir que a animação começa na próxima frame após reflow
        requestAnimationFrame(() => {
            console.log('[goToSlide] Animating new slide in via requestAnimationFrame...');
    ...
    parseCSV(text) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            for (let j = 0; j < line.length; j++) {
                const char = line[j];
                if (char === '"') {
                    if (inQuote && line[j+1] === '"') { // Handle escaped double quote ""
                        currentField += '"';
            ...
            if (cleanedValues.length !== headers.length) {
                console.warn(`[parseCSV] Skipping line ${i + 1} due to mismatched column count: "${line}" (Expected ${headers.length}, Got ${cleanedValues.length}). Please check the CSV data format.`);
                continue;
            }
...
// A instância da aplicação é criada fora do DOMContentLoaded para que possa ser acedida globalmente,
// mas a sua inicialização (init()) é atrasada até o DOM estar pronto.
